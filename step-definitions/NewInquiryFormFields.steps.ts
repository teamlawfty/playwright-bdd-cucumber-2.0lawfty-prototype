...
Given('I am on the New Inquiry form page', async () => {
  await newInquiryPage.navigateToNewInquiryPage(
    process.env.NEW_INQUIRY_URL || '',
  )
})
...
Then(
  'I should see all required fields on the New Inquiry form',
  async (dataTable) => {
    const fieldNames = dataTable.rawTable
      .slice(1)
...
Then(
  ...
  async (dropdown) => {
    await newInquiryPage.verifyDropdownOptions(dropdown, [
      'CALL',
      'CHAT',
      'FORM',
      'REFERED',
...
Then(
  'I should see a confirmation message indicating the inquiry has been successfully created',
  async () => {
    //to be implemented after bug fix
  },
)

...
import {
  ...
  getPage,
  retry,
} from '../helpers/playwrightContext'
...
When(
  ...
  async (buttonText: string, elementType: string) => {
    const page = getPage()
...
Then(
  /^I should see the "(.+)" (field|button|checkbox|link|textarea)$/,
  async (label: string, elementType: string) => {
    const page = getPage()
    let locators: string[] = [] // Array to hold possible locators
    ...
    await retry(async () => {
      ...
      if (!elementFound) {
        throw new Error(
          `Element "${label}" of type "${elementType}" not found using the available locator strategies.`,
        )
      }
    })
...
Then(
  /^I should see (an error|normal) message "([^"]*)"$/,
  async (messageType: string, expectedMessage: string) => {
    const page = getPage()
    await expect(
      page.locator(`//p[contains(text(), "${expectedMessage}")]`),
    ).toBeVisible()
  },
)
...
Then(
  ...
  async (
    ...
    const page = getPage()
    await expect(
      page.locator(`//input[@name="${emailField.toLowerCase()}"]`),
    ).toBeFocused()
    await page.keyboard.press('Tab')
    await expect(
      page.locator(`//input[@name="${passwordField.toLowerCase()}"]`),
    ).toBeFocused()
    await page.keyboard.press('Tab')
    await expect(page.locator(`//button[@role="checkbox"]`)).toBeFocused()
    await page.keyboard.press('Tab')
    await expect(
      page.locator(`//button[@type="${buttonField.toLowerCase()}"]`),
    ).toBeFocused()
  },
)
...
Then(
  /^I should see a dropdown for "([^"]*)" with options:$/,
  async (dropdownName: string, expectedOptionsTable) => {
    const page = getPage()

    const expectedOptions = expectedOptionsTable.raw().flat()

    const dropdownTriggerLocator = page.locator(
      `//button[@id='${dropdownName}']`,
    )
    ...
    const customOptionsContainer = page.locator(
      '//div[@role="listbox" or contains(@class,"dropdown-container")]',
    )
    await customOptionsContainer.waitFor({ state: 'visible', timeout: 5000 })

    await customOptionsContainer.scrollIntoViewIfNeeded()

    const visibleOptions = customOptionsContainer.locator(
      '//div[@role="option" or contains(@class,"dropdown-item")]',
    )
